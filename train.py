...
def namegen():
    """
    Generate a unique name for each run based on hyperparameters.
    """
    lr = 1e-4
    epochs = 5
    name = f"meta-dit-lr-{lr}-epochs-{epochs}"
    return name
...
wandb.init(
    ...
    config={
        "learning_rate": 1e-4,
        "architecture": "DiT-S/2 modified",
        "dataset": "CIFAR-10",
        "epochs": 5,
        "loss_metric": "KL-Divergence",
    },
)
...
def main(args):
    ...
    diffusion = create_diffusion(
        timestep_respacing="", use_kl=True
    ...
    training_data = torch.utils.data.Subset(
        data, np.random.choice(len(data), 48000, replace=False)
    )
    validation_data = torch.utils.data.Subset(
        data, np.random.choice(len(data), 6000, replace=False)
    )
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                wandb.log({"Train Loss": avg_loss, "Train Steps/Sec": steps_per_sec})
        ...
        for x, y in val_loader:
            ...
            if val_steps % 20 == 0:
                ...
                # torch.cuda.synchronize()
                # end_time = time()
                # steps_per_sec = val_log_steps / (end_time - start_time)
                # Reduce loss history over all processes:
                avg_loss = torch.tensor(val_running_loss / val_log_steps, device=device)
                dist.all_reduce(avg_loss, op=dist.ReduceOp.SUM)
                avg_loss = avg_loss.item() / dist.get_world_size()
                logger.info(
                    f"(step={val_steps:07d}) Validation Loss: {avg_loss:.4f}"
                )  # , Validation Steps/Sec: {steps_per_sec:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-size", type=int, choices=[256, 512, 32], default=32)
